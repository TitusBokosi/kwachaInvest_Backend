...
export const getActiveSessionsByUser = async (userId) => {
    return prisma.authSession.findMany({
        ...
        select: {
            ...
            createdAt: true,
            expiresAt: true,
            // tokenHash intentionally excluded — never expose it, even hashed
        },
    });
}
...
/** Housekeeping — run on a schedule (e.g. daily cron) to keep the table small. */
...
export const createOtpCode = async ({ userId, codeHash, purpose, expiresAt }) => {
    return prisma.otpCode.create({
        data: { userId, codeHash, purpose, expiresAt },
    });
}

/** Most recent unconsumed, unexpired OTP for this user + purpose. */
...
/** Invalidate any other outstanding OTPs for this user+purpose before issuing a new one. */

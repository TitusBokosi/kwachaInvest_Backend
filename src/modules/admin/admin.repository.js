...
export const getDashboardStats = async () => {
    ...
    return {
        ...
        savings: {
            ...
            byStatus: savingsByStatus.map((row) => ({ status: row.status, count: row._count })),
            byType: savingsByType.map((row) => ({ type: row.type, count: row._count })),
        },
        transactions: {
            totalSuccessful: successfulTransactions._count,
            totalSuccessfulAmount: Number(successfulTransactions._sum.amount ?? 0),
            byStatus: transactionsByStatus.map((row) => ({ status: row.status, count: row._count })),
        },
    };
}

// ---------------------------------------------------------------------------
// Cross-user savings account oversight
// ---------------------------------------------------------------------------

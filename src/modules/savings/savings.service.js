...
export const updateMySavingsAccountName = async (userId, savingsAccountId, name) => {
    const account = await getOwnedAccountOrThrow(userId, savingsAccountId);

    if (account.status === "CANCELLED" || account.status === "COMPLETED") {
        throw new ValidationError(`Cannot update a savings account that is ${account.status.toLowerCase()}`);
    }
...
export const pauseMySavingsAccount = async (userId, savingsAccountId) => {
    ...
    if (account.status !== "ACTIVE") {
        throw new ValidationError(`Cannot pause an account that is ${account.status.toLowerCase()}`);
    }
...
export const resumeMySavingsAccount = async (userId, savingsAccountId) => {
    ...
    if (account.status !== "PAUSED") {
        throw new ValidationError(`Cannot resume an account that is ${account.status.toLowerCase()}`);
    }
...
export const cancelMySavingsAccount = async (userId, savingsAccountId) => {
    ...
    if (account.status === "CANCELLED" || account.status === "COMPLETED") {
        throw new ValidationError(`This account is already ${account.status.toLowerCase()}`);
    }
 ...
 */
export const getWithdrawalBreakdown = async (userId, savingsAccountId, amount) => {
    const account = await getOwnedAccountOrThrow(userId, savingsAccountId);
    ...
    if (account.status === "CANCELLED") {
        throw new ValidationError("This savings account has already been cancelled");
    }
    ...
    let isEarly = false;
    if (account.withdrawalPolicy === "LOCKED") {
        if (account.type === "TIME_BASED" && account.timeBasedDetails) {
            ...
            isEarly = Number(account.balance) < Number(account.targetBasedDetails.target);
        }
    }

    const penaltyPercentage = isEarly ? Number(account.penaltyPercentage) : 0;
    const penaltyAmount = Math.round(requested * (penaltyPercentage / 100) * 100) / 100;
    const payoutAmount = Math.round((requested - penaltyAmount) * 100) / 100;

    return { account, amount: requested, isEarly, penaltyPercentage, penaltyAmount, payoutAmount };
}

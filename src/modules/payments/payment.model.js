...
// ---------------------------------------------------------------------------
// Saved payment methods (MobileMoneyAccount) — convenience records only,
// created/updated after a successful Direct Charge, never a prerequisite.
// ---------------------------------------------------------------------------
...
/** Upsert on (userId, provider, phoneNumber) — reused each time the same number is charged again. */
export const upsertSavedPaymentMethod = async ({ userId, provider, operatorRefId, phoneNumber, isVerified }) => {
...
export const createPaymentGatewayTransaction = async ({ transactionId, mode, status, gatewayReference, responsePayload }, db = prisma) => {
    return db.paymentGatewayTransaction.create({
        data: { transactionId, mode, status, gatewayReference, responsePayload },
    });
}

export const updatePaymentGatewayTransaction = async (id, data, db = prisma) => {
    return db.paymentGatewayTransaction.update({ where: { id }, data });
}

export const findPaymentGatewayTransactionByTransactionId = async (transactionId) => {
    return prisma.paymentGatewayTransaction.findUnique({ where: { transactionId } });
}

export const findPaymentGatewayTransactionByGatewayReference = async (gatewayReference) => {
    return prisma.paymentGatewayTransaction.findFirst({ where: { gatewayReference } });
}
...
export const createWebhookEvent = async ({ paymentGatewayTransactionId, providerEventId, payload }) => {

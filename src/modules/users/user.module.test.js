...
describe("Users module", () => {
  ...
  describe("repository", () => {
    ...
    it("getUserById returns null when the user does not exist", async () => {
      ...
      await expect(
        usersRepository.getUserById("missing-id"),
      ).resolves.toBeNull();
    });
    ...
    it("getUserByEmailForAuth DOES include passwordHash (auth-only finder)", async () => {
      ...
      const result =
        await usersRepository.getUserByEmailForAuth("tee@example.com");
    ...
    it("existsByEmailOrPhone returns true/false based on count", async () => {
      ...
      await expect(
        usersRepository.existsByEmailOrPhone(
          "nobody@example.com",
          "0000000000",
        ),
      ).resolves.toBe(false);
  ...
  describe("service", () => {
    ...
    it("registerUser throws ConflictError when email/phone already exists", async () => {
      ...
      await expect(
        usersService.registerUser(validRegisterInput),
    ...
    it("registerUser hashes the password and creates the user with a derived fullName", async () => {
      ...
      expect(result).toMatchObject({ id: "u1", email: "tee@example.com" });
    });

    it("getUserById throws NotFoundError when the user doesn't exist", async () => {
      prisma.user.findUnique.mockResolvedValue(null);

      await expect(usersService.getUserById("missing")).rejects.toThrow(
        NotFoundError,
      );
    });
    ...
    it("updateUser strips non-whitelisted fields before writing", async () => {
      ...
      await usersService.updateUser("u1", {
        firstName: "Teddy",
        isActive: false,
      });
    ...
    it("updateUser throws ValidationError when no whitelisted fields are provided", async () => {
      ...
      await expect(
        usersService.updateUser("u1", { isActive: false }),
      ).rejects.toThrow(ValidationError);
    ...
    it("changePassword hashes and saves the new password on success", async () => {
      ...
      expect(hashUtil.compareValue).toHaveBeenCalledWith(
        "current-password",
        "old-hash",
      );
      expect(prisma.user.update).toHaveBeenCalledWith({
    ...
    it("updateUserRole rejects a no-op role change", async () => {
      ...
      await expect(
        usersService.updateUserRole("admin1", "u2", "USER"),
      ).rejects.toThrow(ValidationError);
    });
  ...
  describe("controller", () => {
    it("register returns 201 with the created user", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({
        success: true,
        data: expect.objectContaining({ id: "u1", email: "tee@example.com" }),
      });
  ...
  describe("validation", () => {
    ...
    it("updateMeSchema rejects an empty body", () => {
      expect(userValidation.updateMeSchema.body.safeParse({}).success).toBe(
        false,
      );
    });
    ...
    it("userIdParamSchema rejects a non-cuid id", () => {
      expect(
        userValidation.userIdParamSchema.params.safeParse({ id: "not-a-cuid" })
          .success,
      ).toBe(false);
    });
    ...
    it("updateUserRoleSchema rejects a role outside USER/ADMIN", () => {
      expect(
        userValidation.updateUserRoleSchema.body.safeParse({
          role: "SUPERADMIN",
        }).success,
      ).toBe(false);
    });
    ...
    it("listUsersSchema rejects a pageSize above the 100 cap", () => {
      expect(
        userValidation.listUsersSchema.query.safeParse({ pageSize: "500" })
          .success,
      ).toBe(false);
    });
  });

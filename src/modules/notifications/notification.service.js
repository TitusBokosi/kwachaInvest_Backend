...
const money = (amount) =>
  `MWK ${Number(amount).toLocaleString('en-MW', { minimumFractionDigits: 2 })}`;
...
export const sendWithdrawalSuccessEmail = async (
  user,
  { savingsAccountName, amount, penaltyAmount },
) => {
  const penaltyNote =
    penaltyAmount > 0
...
export const sendSavingsFrozenEmail = async (
  user,
  { savingsAccountName, reason },
) => {
...
export const sendSavingsUnfrozenEmail = async (
  user,
  { savingsAccountName },
) => {
  await send({
